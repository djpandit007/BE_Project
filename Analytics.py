...
def scatterPlot():
	...
	# print area
	wordData = words.posts.find({}, {'word':1, 'diff':1, 'points':1})
	length = []
	points = []
	diff = []
	for w in wordData:
		length.append(len(w['word']))
		points.append(w['points'])
		diff.append(w['diff'])

	plt.scatter(points, diff, c=colors, alpha=0.5, edgecolors='face')
	plt.show()